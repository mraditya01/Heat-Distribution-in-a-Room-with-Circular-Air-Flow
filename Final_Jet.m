...
c=1.5; %velocity constant -> a
b=0.004; %velocity constant -> b
Nx=500; %Number of x-Tiles
Ny=500; %Number of y-Tiles
Lx=8.0; %x-axis Length of the room [m]
Ly=8.0; %y-axis Length of the room [m]
Lh=2; %Length of the heater (x-by-x) [m]
Center=[0.0]; %Center of Heater x-coordinate [m]
...
for j=3:Ny+2
	if x(i)^2+y(j)^2>=c^2
		if x(i)<=Lh/2 && x(i)>=-Lh/2
		Vy=b*c^2*x(i)/(y(j)^2+x(i)^2)-0.001*(1-(x(i)/(Lh/2))^2)*(y(j)+Ly/2);
		Vx=b*c^2*y(j)/(y(j)^2+x(i)^2)*-1;
		else
        Vx=b*c^2*y(j)/(y(j)^2+x(i)^2)*-1;
        Vy=b*c^2*x(i)/(y(j)^2+x(i)^2);
		end
        Vxx(i,j)=Vx; %for velocity distribution graph
        Vyy(i,j)=Vy; %for velocity distribution graph
        Vr(i,j)=sqrt(Vx^2+Vy^2);
	elseif x(i)^2+y(j)^2<c^2
		if x(i)<=Lh/2 && x(i)>=-Lh/2
		Vy=b*x(i)-0.001*(1-(x(i)/(Lh/2))^2)*(y(j)+Ly/2);
		Vx=b*y(j)*-1;
		else
		Vx=b*y(j)*-1;
		Vy=b*x(i);
